...
public abstract class BuildMenuTab<E extends IBuildComponent<?>> extends ImageButton {
    ...
    public void renderTexture(GuiGraphics guiGraphics, ResourceLocation texture, int x, int y, int uOffset, int vOffset, int textureDifference, int width, int height, int textureWidth, int textureHeight) {
        ...
        if (!this.isHoveredOrFocused()) {
            ...
            screen.getVertexContainer().putBliz(resourceLocation, TextureBufferInfo.of((getX() + (getWidth() / 2.0f) - 4), getY(), getHeight(), getHeight(), 0, 0, 32, 32, 32, 64, pose.last().pose()));
            //guiGraphics.blit(resourceLocation, (int) (getX() + (getWidth() / 2.0f) - 4), getY(), getHeight(), getHeight(), 0, 0, 32, 32, 32, 64);
            guiGraphics.setColor(1, 1, 1, 1);
        } else {
            screen.getVertexContainer().putBliz(resourceLocation, TextureBufferInfo.of((int) (getX() + (getWidth() / 2.0f) - 5), getY() - 1, getHeight() + 1, getHeight() + 1, 0, 32, 32, 32, 32, 64, pose.last().pose()));
    ...
    *///?} else {
    ...
    public void renderWidget(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        if (hoveredOrFocused) {
            ...
            screen.getVertexContainer().putBliz(resourceLocation, TextureBufferInfo.of((int) (getX() + (getWidth() / 2.0f) - 4), getY(), getHeight(), getHeight(), 0, 0, 32, 32, 32, 32, pose.last().pose()));

        } else {
            screen.getVertexContainer().putBliz(resourceLocation, TextureBufferInfo.of((int) (getX() + (getWidth() / 2.0f) - 5), getY() - 1, getHeight() + 1, getHeight() + 1, 0, 32, 32, 32, 32, 32, pose.last().pose()));
        }
        pose.popPose();
    }
